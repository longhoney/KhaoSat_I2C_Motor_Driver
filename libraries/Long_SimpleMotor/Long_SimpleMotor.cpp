...
void Long_SimpleMotor::setup(uint8_t paI2cAddress) {    // kế thừa setup(...)
  ...
  // i2cMotorDriver = new Long_I2C_Motor_Driver(paI2cAddress);
  i2cMotorDriver = new Makerlabvn_I2C_Motor_Driver(paI2cAddress);
  i2cMotorDriver->begin();    // Dẫn đến hay trỏ đến begin();
}
...
void Long_SimpleMotor::motorA_bw(int speed)
{
  // Xử lý giá trị tốc độ nhận được
  speed = calculate_speed(speed);
  this->lastSpeedA = -speed;    // gán số âm
...
int Long_SimpleMotor::calculate_speed(int speed)    //Dữ liệu người dùng nhập vào
{
  ...
  speed = map(speed, 0, 100, 0, 255);   // https://docs.arduino.cc/language-reference/en/functions/math/map/
  // AnalogRead (0-1024) // AnalogWrite (0-255)
